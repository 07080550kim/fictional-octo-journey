          ...
          groupedMessages.map((group, gi) => (
            <div key={gi}>
              ...
              {group.messages.map((msg) => {
                const isMine = String(msg.senderId) === String(user?.id) || String(msg.sender?.id) === String(user?.id);
                const senderName = msg.sender?.displayName || msg.sender?.username || 'Пользователь';
                ...
                return (
                  <div
                    ...
                    className={`flex mb-3 message-enter ${isMine ? 'justify-end' : 'justify-start'} items-end gap-2 transition-all duration-300`}
                  >
                    ...
                    >
                      ...
                      {msg.forwardFromSender && (
                        <div className="text-[11px] italic text-acid-cyan/80 mb-1 flex items-center gap-1">
                      ...
                      {!isMine && activeChat?.type !== 'private' && (
                        <div className="text-xs font-bold text-acid-pink mb-1">
                          {senderName}
                        </div>
                      )}
                      ...
                      {/* Reply */}
                      {msg.replyTo && (
                        <div className="mb-2 pl-2 border-l-2 border-acid-green/50 bg-white/[0.03] p-1 rounded-r text-xs opacity-90 cursor-pointer" onClick={() => msg.replyToId && scrollToMessage(msg.replyToId)}>
                          <div className="font-bold text-acid-green">{msg.replyTo.sender?.displayName || 'Пользователь'}</div>
                          <div className="truncate text-gray-400">{msg.replyTo.text}</div>
                        </div>
                      )}

                      {/* Медиа */}
                      {msg.mediaUrl && msg.type === 'image' && (
                        <img src={msg.mediaUrl} alt="" className="max-w-full rounded-lg mb-2 max-h-80 object-cover cursor-zoom-in" />
                      )}
                      {msg.mediaUrl && msg.type === 'file' && (
                        <a href={msg.mediaUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3 p-2.5 bg-black/30 rounded-xl mb-2 hover:bg-black/40 transition-colors border border-white/5">
                          <div className="w-10 h-10 rounded-lg bg-acid-cyan/20 flex items-center justify-center text-acid-cyan">
                            <FileText className="w-6 h-6" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-sm font-medium truncate">{msg.sender?.displayName || 'Файл'}</div>
                            <div className="text-[10px] text-gray-500 uppercase">Скачать файл</div>
                          </div>
                        </a>
                      )}
